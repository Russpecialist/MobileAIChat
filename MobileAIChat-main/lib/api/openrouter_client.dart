...
class OpenRouterClient {
  ...
  bool get isConfigured => _apiKey != null && _baseUrl != null;

  static String detectProviderFromKey(String apiKey) {
    return apiKey.startsWith('sk-or-v1-') ? 'openrouter' : 'vsegpt';
  }

  static String getBaseUrlByProvider(String provider) {
  ...
  Future<double?> getBalanceValue() async {
    ...
    final response = await http.get(
      Uri.parse(_baseUrl!.contains('vsegpt.ru')
          ? '$_baseUrl/balance'
          : '$_baseUrl/credits'),
      headers: headers,
    );
    ...
    if (_baseUrl!.contains('vsegpt.ru')) {
      return double.tryParse(data['data']?['credits']?.toString() ?? '0');
    }
    final credits = (data['data']?['total_credits'] ?? 0).toDouble();
    final usage = (data['data']?['total_usage'] ?? 0).toDouble();
